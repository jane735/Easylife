...
EasyLife 是一个类“大众点评”的智慧周边服务平台。采用前后端分离架构，支持短信验证码登录、商户管理、优惠券秒杀、探店笔记以及智能推荐Agent等。

### 核心模块：
- 数据一致性：基于 Redis + Lua 脚本实现库存扣减与一人一单的原子性校验，解决秒杀场景的超卖与重复下单问题；
- 业务解耦：引入 RocketMQ 消息队列，将下单流程异步化，实现核心链路流量削峰，提升系统响应速度；
- 限流安全：采用互斥锁+逻辑过期策略应对热点Key缓存击穿，通过缓存空值结合布隆过滤器预防缓存穿透；
- 定时任务：基于延迟队列实现分钟级定时任务，自动关闭超时未支付订单，及时释放库存资源，提升系统资源利用率；

### 新增AI模块：

- 基于 Milvus 构建知识库、店铺画像、评论数据三类向量索引，打通多源向量检索链路。
- 在 RAG 检索环节结合向量召回与关键词，提升平台规则问答、门店推荐等场景下的召回率与稳定性。
- 基于 Spring AI 实现多轮 Think-Execute 推理链路，支持工具调用、任务终止、异常兜底，并预留外部工具扩展能力（MCP Client）。
- 自动下单能力：在门店推荐与用户确认后，通过调用订单服务接口完成自动下单；支持从对话中提取商品信息、收货地址、支付方式等关键参数，并在下单前后触发风控校验与库存预占。
...
### 8. RAG 检索
   ...
   **检索策略：** 我起初使用的是纯向量检索，语义理解能力强。但是我在测试的时候发现AI经常无法命中我保存在数据库中的规则，比如我设置了超级会员的福利，当用户通过AI提问时经常无法命中。于是我想办法让这些高确定性的问题比较容易命中，最终确定了
